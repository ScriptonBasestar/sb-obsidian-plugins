...
export class LLMService {
  ...
  private buildLegacyPrompt(context: CommitContext): string {
    ...
    let prompt =
      this.settings.commitPrompt || 'Generate a concise commit message for these changes:';
    prompt += '\n\n';
    ...
    if (files.staged.length > 0) {
      prompt += `Staged: ${files.staged.slice(0, 5).join(', ')}${
        files.staged.length > 5 ? ` and ${files.staged.length - 5} more` : ''
      }\n`;
    }

    if (files.unstaged.length > 0) {
      prompt += `Modified: ${files.unstaged.slice(0, 5).join(', ')}${
        files.unstaged.length > 5 ? ` and ${files.unstaged.length - 5} more` : ''
      }\n`;
    }

    if (files.untracked.length > 0) {
      prompt += `New: ${files.untracked.slice(0, 5).join(', ')}${
        files.untracked.length > 5 ? ` and ${files.untracked.length - 5} more` : ''
      }\n`;
    }
