...
import { ScriptonSyncSettings, BranchStrategy } from './types';

export class ScriptonSyncSettingTab extends PluginSettingTab {
  plugin: ScriptonSyncPlugin;
  ...
  private renderSyncStatus(container: HTMLElement): void {
    ...
    statusEl.createSpan({
      cls: `sync-status-indicator ${status.syncing ? 'syncing' : status.hasChanges ? 'pending' : 'synced'}`,
    });
  ...
  private renderGitSettings(container: HTMLElement): void {
    ...
    if (this.plugin.settings.branchStrategy === 'develop-host') {
      new Setting(container)
        ...
        .addText(text => text
          .setValue(this.plugin.settings.tempBranchPrefix)
          .onChange(async (value) => {
      ...
      new Setting(container)
        ...
        .addToggle(toggle => toggle
          .setValue(this.plugin.settings.autoMergeToDefault)
          .onChange(async (value) => {
